    ...
    internal class Program
    {
        ...
        private static async Task ClientDisconnected(ClientObject client)
        {
            ...
            { 
                ...
                if (!client.CurrentRoom.IsGameActive())
                {
                    rooms.TryRemove(client.CurrentRoom.Name, out _);
                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine($"Кімната '{client.CurrentRoom.Name}' видалена (гра завершена або не розпочата)");
                    Console.ResetColor();
                }
        ...
        private static async Task ProcessMessageAsync(ClientObject client, string message)
        {
            ...
            switch (command)
            {
                ...
                case "CREATE_ROOM":
                    ...
                    {
                        var existingRoom = rooms[roomName];
                        if (!existingRoom.IsGameActive() && !existingRoom.IsFull)
                        {
                            rooms.TryRemove(roomName, out _);
                            Console.ForegroundColor = ConsoleColor.Yellow;
                            Console.WriteLine($"Видалена стара неактивна кімната '{roomName}'");
                            Console.ResetColor();
                        }
                        else
                        {
                ...
                case "JOIN_ROOM":
                    ...
                    if (rooms.TryGetValue(targetRoom, out var room))
                    {
                        ...
                        { 
                            ...
                            Console.ForegroundColor = ConsoleColor.Green;
                            Console.WriteLine($"{client.Nickname} приєднався до кімнати '{targetRoom}'");
                            Console.ResetColor();
                        }
                        else
                        {
                    ...
                    {
                        await client.SendMessageAsync("ERROR|Кімната не знайдена");
                    }
                    break;
                ...
                case "GET_LEADERBOARD":
                    string leaderboard = LeaderboardManager.GetLeaderboard();
                    await client.SendMessageAsync(leaderboard);
                    break;
            }
        }
        ...
        {
            var openRooms = rooms.Values.Where(r => !r.IsFull && !r.IsGameActive()).Select(r => r.Name);
